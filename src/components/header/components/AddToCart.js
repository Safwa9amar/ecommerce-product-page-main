...
const AddToCart = ({ cardData, removeItem }) => {
  ...
  return (
    <a href="#nav" className="cart">
      <div className="cart__btn" onClick={handleShow}>
        <div className="cart__indicator">{cardData.length}</div>
...
const CartItem = ({ id, img, header, price, amount, removeItem }) => {
  ...
  return (
    <div className="cartItem">
      <img className="cardItem__thumb" src={img} alt="product thumb" />
      <div className="cartItem__detaills">
        <p className="cartItem__name">{header}</p>
        <p className="cartItem__price">
          ${price} x {amount} <strong>${price * amount}.00</strong>
        </p>
      </div>
      <div onClick={handleRemove} className="cartItem--delete">
